...
//bool EllipsoidIntersectionShader(Ellipsoid el, float3 r0, float3 rd, out float tIn, out float tOut)
//{    
//    //Given ray: r0 + t*rd, substitute as (x,y,z) in ellipsoid equation: x^2/rx^2 + y^2/ry^2 + z^2/rz^2 = 1 (rx,ry,rz = ellipsoid radii)
//    float3 scale = el.radii * el.extent;
//    float3 r0N = mul((float3x3)transpose(el.rot), r0 - el.center) / scale; 
//    float3 rdN = mul((float3x3)transpose(el.rot), rd) / scale;
...
void MyComputeShader(uint3 DTid : SV_DispatchThreadID)
{
    ...
    // Write the raytraced color to the output texture.
    RenderTarget[DTid.xy] = float4(1, 1, 1, 1);
}
